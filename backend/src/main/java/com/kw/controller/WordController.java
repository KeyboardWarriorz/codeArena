...
public class WordController {
	 ...
	 */
	@PostMapping("/word")
	public String insert(UserWord userWord) {
		
		// 1. 사용자 User_Word DB에 같은 내용이 이미 존재하는 지 확인한다.
		
		// 2-1. 이미 있다면 "이미 단어사전에 등록되어 있습니다." 출력한다.
		
		// 2-2. 없다면 공용 Word DB에서 정보(word_id) 가져와 User_Word DB에 값을 저장한다. 

		return "redirect:/"; // 기존에 보고 있던 페이지에 그대로
	}
	 ...
	 */
	@GetMapping("/word")
	public ResponseEntity<responseApi<String>> selectBoardAll(String name) {
		responseApi<String> response = new responseApi<String>();

		// 1. 공용 Word DB에 해당하는 단어가 이미 있는 지 확인한다.
		Word word = wordService.selectByName(name);
		...
		if (word == null) {
			...
			if(description == null) { //chatGPT에서 가져온 값이 없다면 
				...
				response.setData(""); //null이여도 null로 보내야하는 가????????????????????????
			} else {
				...
				saveWord.setName(name);
				saveWord.setDescription(description);
				
				wordService.insert(saveWord); //저장 
			}
