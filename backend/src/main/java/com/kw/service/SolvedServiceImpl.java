	...
	public List<SolvedDTO> selectSolved_user(String userId, int success) {
		...
		for(Solved s : sol) {
			...
			if(s.getSuccess() == success) {	
				...
				P_dto = new ProblemDTO(p.getProblemId(),p.getTitle(),p.getQuestion(),p.getAnswerIndex(),p.getAnswer1(),p.getAnswer2(),p.getAnswer3(),p.getAnswer4(),p.getSolution(),problem_type,p.getSubcategory());
